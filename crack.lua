...
-- Đặt trước khi SetIcon được gọi
local defaultTitle = ae and ae.Title or "Open VTriP"
...
function af.SetIcon(am, an)
    ...
    if an then
        ag = ab.Image(
            an,
            defaultTitle,  -- dùng title mặc định hoặc từ ae.Title
            0,
            ae.Folder,
            "OpenButton",
            true,
...
ar.MainOpenButton = a.load'x'.New(ar)

task.spawn(function()
    if ar.Icon then
        local windowIcon = ai.Image(
            ar.Icon,
            ar.Title,
            0,
            ar.Folder,
        ...
        windowIcon.Parent = ar.UIElements.Main.Main.Topbar.Left
        windowIcon.Size = UDim2.new(0, ar.IconSize, 0, ar.IconSize)

        ar.MainOpenButton:SetIcon(ar.Icon)
    else
        ar.MainOpenButton:SetIcon(ar.Icon)
    end
end)
