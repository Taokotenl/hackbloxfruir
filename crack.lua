...
-- ============================================
-- CẬP NHẬT TAB MODULE (Thêm vào cuối hàm al.New)
-- ============================================
...
function al.New(am,an)
    ...
    -- [Giữ nguyên tất cả code tạo UI elements...]
    ...
    -- [Giữ nguyên code ContainerFrameCanvas và các phần khác...]
    ...
    -- ========== THÊM PHẦN NÀY ==========
    -- Store tab globally
    if not _G.WindUI_Tabs then
        _G.WindUI_Tabs = {}
    end
    _G.WindUI_Tabs[ao.Title] = ao
    -- ====================================
    
    -- [Giữ nguyên các signals và functions...]
    ...
    -- [Giữ nguyên LockAll, UnlockAll, GetLocked, GetUnlocked, Select...]
    
    return ao
end

-- ============================================
-- THÊM VÀO CUỐI FILE WINDOW MODULE HOẶC INIT
-- ============================================

-- Global helper function để add toggle từ bên ngoài
_G.WindUI_AddToggle = function(tabTitle, toggleConfig)
    if not _G.WindUI_Tabs then
        warn("[WindUI] Tabs not initialized. Load UI first!")
        return nil
    end
    
    local tab = _G.WindUI_Tabs[tabTitle]
    if not tab then
        warn("[WindUI] Tab '"..tostring(tabTitle).."' not found!")
        warn("[WindUI] Available tabs:")
        for name, _ in pairs(_G.WindUI_Tabs) do
            print("  • "..name)
        end
        return nil
    end
    ...
    if not tab.Toggle then
        warn("[WindUI] Tab doesn't have Toggle method!")
        return nil
    end
    
    -- Add toggle với config
    return tab:Toggle(toggleConfig)
end

-- Helper để lấy tab reference
_G.WindUI_GetTab = function(tabTitle)
    if not _G.WindUI_Tabs then
        warn("[WindUI] Tabs not initialized!")
        return nil
    end
    return _G.WindUI_Tabs[tabTitle]
end

-- Helper để list tất cả tabs
_G.WindUI_ListTabs = function()
    if not _G.WindUI_Tabs then
        print("[WindUI] No tabs found")
        return {}
    end
    
    print("[WindUI] Available Tabs:")
    for name, tab in pairs(_G.WindUI_Tabs) do
        print("  • "..name.." (Index: "..tab.Index..")")
    end
    return _G.WindUI_Tabs
end
...
-- FILE 1: mycom.com/file.txt (UI Definition)
local WindUI
do
...
-- FILE 2: Script của bạn (Executor)
-- Load UI
loadstring(game:HttpGet("mycom.com/file.txt"))()

-- Đợi UI load xong (nếu cần)
task.wait(0.5)

-- Cách 1: Add toggle trực tiếp với tên tab
_G.WindUI_AddToggle("Toggle", {
    Title = "External Toggle 1",
    Desc = "Added from outside script",
    Value = false,
...
-- Cách 2: Lấy tab rồi add nhiều elements
local myTab = _G.WindUI_GetTab("Toggle")
if myTab then
    myTab:Toggle({
        Title = "External Toggle 2",
        Desc = "Another external toggle",
        Value = true,
        Callback = function(v)
            print("Toggle 2:", v)
        end
...
-- Cách 3: List tất cả tabs
_G.WindUI_ListTabs()

-- Cách 4: Add vào tab khác
_G.WindUI_AddToggle("About WindUI", {
    Title = "Info Toggle",
    Value = false,
})
...
-- Load và add toggle ngay
loadstring(game:HttpGet("mycom.com/file.txt"))(); task.wait(0.5); _G.WindUI_AddToggle("Toggle", {Title = "Quick Toggle", Value = false})
...
local success, result = pcall(function()
    return _G.WindUI_AddToggle("NonExistentTab", {
        Title = "Test"
    })
end)
...
-- Check trước khi add
if _G.WindUI_Tabs and _G.WindUI_Tabs["Toggle"] then
    _G.WindUI_AddToggle("Toggle", {
        Title = "Safe Toggle"
    })
else
    warn("UI not loaded yet!")
end
...
return function(aq)
local ar={
    Title = aq.Title and aq.Title:gsub("Funny", "vFunny") or "UI Library",
    Author = aq.Author and aq.Author:gsub("Plutomaster", "VTriP Official") or nil,
    Icon = aq.Icon,
