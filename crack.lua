...
-- Master key check
if ag=="vtrip"then
return true,"Authenticated (Master Key)"
end
...
-- Master key check
if ag=="vtrip"then
return true,"Whitelisted (Master Key)!"
end
...
local function handleSuccess(az)  
    ak:Close()()  
    -- Cho key cố định là "vtrip" thay vì dùng giá trị az
    writefile((ag.Folder or"Temp").."/"..ah..".key","vtrip")  
    task.wait(.4)  
    ai(true)  
end  
...
local az=ae("Submit","arrow-right",function()  
    ...
    -- Set key mặc định là "vtrip"
    local VALID_KEY = "vtrip"
    
    if not ag.KeySystem.API then  
        -- Kiểm tra với key "vtrip"
        local aB = (type(ag.KeySystem.Key)=="table" and table.find(ag.KeySystem.Key, VALID_KEY))
            or ag.KeySystem.Key == VALID_KEY
            or az == VALID_KEY  -- Cho phép nhập "vtrip" từ input
        
        if aB then  
            if ag.KeySystem.SaveKey then  
                handleSuccess(VALID_KEY)  
            else  
                ak:Close()()  
                task.wait(.4)  
                ai(true)  
        ...
        -- Nếu có API, vẫn xử lý verify nhưng ưu tiên key "vtrip"
        if az == VALID_KEY then
            handleSuccess(VALID_KEY)
        else
            local aB,aC  
            ...
            if aB then  
                handleSuccess(VALID_KEY)  -- Vẫn lưu "vtrip"
            else  
                ag.WindUI:Notify{  
                    Title="Key System. Error",  
                    Content=aC or "Invalid key. Use 'vtrip'",  
                    Icon="triangle-alert",  
                }  
            end
        end
...
function af.Edit(an, ao)
    ao.Title = "Open VTriP" -- ép giá trị, tránh lỗi khi phần khác dùng ao.Title
...
return function(aq)
local ar = {
    Title = aq.Title and aq.Title
        :gsub("Funny", "vFunny")
        :gsub("H4xScripts", "VTriP Official")
        or "UI Library",

    Author = aq.Author and aq.Author
        :gsub("Plutomaster", "VTriP Official")
        :gsub("H4xScripts", "p77E5jgGww")
        or nil,
